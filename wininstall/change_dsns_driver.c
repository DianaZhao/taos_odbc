/************************************************************************************
   Copyright (C) 2019 TAOS Corporation
...
const char OldVersionsDriverName[][32] = {"TAOS ODBC 1.0 Driver", "TAOS ODBC 2.0 Driver"};
const char *DriverVersionBeingInstalled = "TAOS ODBC 3.0 Driver";
const char *OldVersionsString = "TAOS ODBC 1.0 Driver, TAOS ODBC 2.0 Driver";
...
typedef struct {
    ...
    size_t PrefixLen;
    SQLRETURN ReturnValue;
    TAOS_ERROR *ErrRecord;
    /* Order number of last requested error record */
    unsigned int ErrorNum;
} TAOS_Error;

SQLRETURN
TAOS_SetError(TAOS_Error *Error, unsigned int SqlErrorCode, const char *SqlErrorMsg, unsigned int NativeError) {
    return SQL_SUCCESS;
}

TAOS_ERROR TAOS_ErrorList[] =
        {
                {"00000", "", "", SQL_ERROR}
        };
...
HKEY TD_OpenRegKey(HKEY Key, const char *Path, BOOL Fatal) {
    HKEY Handle = NULL;
...
int DoSingleRegBranch(HKEY Branch, const char *NewDriverName) {
    ...
    OdbcIni = TD_OpenRegKey(Branch, "Software\\ODBC\\ODBC.INI", TRUE);
    ...
    OdbcDataSources = TD_OpenRegKey(Branch, "Software\\ODBC\\ODBC.INI\\ODBC Data Sources", TRUE);

    for (i = 0; i < TargetDsnCount; ++i) {
        _snprintf(DsnSubkey, sizeof(DsnSubkey), "Software\\ODBC\\ODBC.INI\\%s", DsnToChange[i]);
        OdbcIni = TD_OpenRegKey(Branch, DsnSubkey, FALSE);
        if (OdbcIni != NULL) {
...
void DoOdbcWay(const char *NewDriverName) {
    char DsName[SQL_MAX_DSN_LENGTH], DriverName[256];
    ...
    while (rc != SQL_ERROR && rc != SQL_NO_DATA) {
        if (DriverToChange(DriverName)) {
            TAOS_Dsn *Dsn = TAOS_DSN_Init();

            if (Dsn == NULL) {
                Die("Could not allocate memory");
            }
            Dsn->DSNName = _strdup(DsName);
            if (!TAOS_ReadDSN(Dsn, NULL, FALSE)) {
        ...
        rc = SQLDataSources(Env, Direction, DsName, sizeof(DsName), &DsnLen, DriverName, sizeof(DriverName),
                            &DriverLen);
    }
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        Usage();
        if (Ask("Are you sure you want to proceed with default parameter values?", "yn", 0)) {
            return 0;
        }
    }
