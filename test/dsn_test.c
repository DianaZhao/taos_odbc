        ...
        if (testCmd) {
            ...
            if (DsnFunc = (DSNDialog) GetProcAddress(hmod, "ConfigDSN")) {
                ...
                if (ret != FALSE) {
                    ...
                    ret = DsnFunc(NULL, ODBC_CONFIG_DSN, driver,
                                  "DSN=dsn_test\0UID=garbage\0PWD=DoubleGarbage\0OPTIONS=2\0\0");
                    ...
                    ret = DsnFunc(hWnd, ODBC_ADD_DSN, driver,
                                  "DSN=dsn_test\0UID=garbage\0PWD=DoubleGarbage\0OPTIONS=2\0\0");
                    printf("%s 5 Replace Prompt \n", ret ? "not ok" : "ok");
                    ret = DsnFunc(NULL, ODBC_CONFIG_DSN, driver,
                                  "DSN=inexistent_dsn\0UID=garbage\0PWD=DoubleGarbage\0OPTIONS=2\0\0");
